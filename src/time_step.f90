subroutine time_step(time,u,nlk,work,mask,mask_color,us,Insect,params_file)
  use vars
  use p3dfft_wrapper
  use insect_module
  implicit none
  ...
  if (index(inicond,'backup::')==0 .and. dry_run_without_fluid/="yes") then
    ...
    call write_integrals(time,u,nlk,work,mask,mask_color,us,Insect)
  endif
  ...
  do while ((time%time<tmax).and.(time%it<=nt).and.(continue_timestepping))
     ...
     if (((modulo(time%time,tsave)<time%dt_new).and.(time%time>=tsave_first)).or.(time%time==tmax)) then
        ...
        call save_fields(time,u,nlk,work,mask,mask_color,us,Insect)
     endif
     ...
     if(idobackup==1 .and. truntimenext<(MPI_wtime()-time_total)/3600.d0) then
         call dump_runtime_backup(time,nbackup,u,Insect)
