...
subroutine SolidSolverWrapper ( time, dt, beams )
  ...
  do i = 1, nBeams
      ...
      if (Vector_isNAN(beams(i)%pressure_new).or.&
        ...
        ! time to go..
        call abort("SolidSolver: input values contain NaNs")
      endif
      ...
      if (time>=T_release) then
        ...
        select case (TimeMethodSolid)
          ...
          case default
            call abort("SolidSolverWrapper::invalid value of TimeMethodSolid"//TimeMethodSolid)
        end select
      else
        !-------------------------------------------
  ...
  do while (iterate)
    ...
    if (mod(iCalls,4607)==0) then
      ...
      if (maxval(J2_norm)>1.0e-2) then
          ...
          write (*,*) time, iter
          close (14)
          call abort("IBES error, mismatch in Jacobian (numeric vs analytic)")
      endif
      iCalls = 0
    endif
...
subroutine Solve_LGS ( J, F, x)
  ...
  if (error .ne. 0) then
    ...
    call abort("!!! Crutial: dgetrf error.")
  endif
  ...
  if (error .ne. 0) then
    ...
    call abort("!!! Crutial: dgetrs error.")
  endif
...
subroutine Tension ( time, T, T_s, theta, theta_dot, pressure, tau_beam, beam_solid)
  ...
  call abort('!!! MKL linear solver was experiencing trouble.')
  endif
...
subroutine Differentiate1D (f, f_derivative, N, dx, order)
  ...
  select case (order)
    ...
    case default
      ...
      call abort("Differentiate1D wrong choice")
  end select
...
subroutine lapack_unit_test()
  ...
  err = maxval( x - b )
  if ( err>1.d-12 ) then
    call abort("LAPACK unit test failed")
  endif
