  ...
  do is=0,ns-1
    do ih=0,nh
      ...
      if((x(1)<-dx).or.(x(1)>xl+dx).or.(x(2)<-dy).or.(x(2)>yl+dy).or.(x(3)<-dz).or.(x(3)>zl+dz)) then
        call abort("ERROR: Surface constuction yielded values outside valid bound.")
      endif
      endif
    enddo
  enddo
...
real(kind=pr) function z_top(s)
  ...
  case("fish_contracting")
    z_top = 0.5d0*1.05d0 - 0.525d0*s**2
  ...
  case default
    call abort("ERROR. plate_shape not defined.."//plate_shape)
  end select
end function
...
real(kind=pr) function z_bottom(s)
  ...
  case("fish_contracting")
    z_bottom = 0.5d0*1.05d0 - 0.525d0*s**2
