...
! files.
!-------------------------------------------------------------------------------
subroutine save_fields(time,u,nlk,work,mask,mask_color,us,Insect)
  use vars
  use p3dfft_wrapper
  use basic_operators
  use insect_module
  ...
  if (isaveMask == 1 .and. iPenalization == 1) then
    ...
    if(iMoving==1) call create_mask( time%time, mask, mask_color, us, Insect, 0 )
...
! precision)
subroutine dump_runtime_backup(time,nbackup,u,Insect)
  use vars
  use hdf5
  use insect_module
  implicit none
...
subroutine read_runtime_backup(filename,time,u,Insect)
  use vars
  use hdf5
  use insect_module
  implicit none
